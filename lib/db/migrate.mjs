...
const migrations = [
  ...
  // Personal schedule subjects: student-created subjects for their timetable
  `CREATE TABLE IF NOT EXISTS personal_schedule_subjects (
    id serial PRIMARY KEY,
    user_id integer NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    name text NOT NULL,
    color text NOT NULL DEFAULT '#5A2D82',
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE(user_id, name)
  )`,
  ...
  "CREATE INDEX IF NOT EXISTS idx_personal_subjects_user ON personal_schedule_subjects(user_id)",

  // Add personal_subject_id to weekly_schedule_slots and make subject_id nullable
  "ALTER TABLE weekly_schedule_slots ADD COLUMN IF NOT EXISTS personal_subject_id integer REFERENCES personal_schedule_subjects(id) ON DELETE CASCADE",
  "ALTER TABLE weekly_schedule_slots ALTER COLUMN subject_id DROP NOT NULL",

  // Indexes for conflict checking and fast lookups
  "CREATE INDEX IF NOT EXISTS idx_wss_user_day ON weekly_schedule_slots(user_id, day_of_week)",
  "CREATE INDEX IF NOT EXISTS idx_wss_personal_subject ON weekly_schedule_slots(personal_subject_id)",
  "CREATE INDEX IF NOT EXISTS idx_wss_user_day_time ON weekly_schedule_slots(user_id, day_of_week, start_time)",
