...
async function runInitMigration() {
  const sqlFile = path.join(__dirname, 'lib', 'db', 'drizzle', '0000_init.sql');
  ...
  // Now run migrate.mjs additional migrations
  if (failed === 0) {
    console.log('Running additional migrations from migrate.mjs...');
  }
